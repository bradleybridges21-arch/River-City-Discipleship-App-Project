...
-- groups (now memberships exists, so this reference is safe)
...
-- responses: owner + leader of that member's group
...
create policy "responses: leader of group can read"
  ...
  using (
    exists (
      select 1
      from teachings t
      join memberships m_member on m_member.group_id = t.group_id
        and m_member.user_id = responses.user_id
      join memberships m_leader on m_leader.group_id = t.group_id
        and m_leader.user_id = auth.uid()
...
-- journal_entries: THE INNER ROOM — owner only, no exceptions
...
-- ── 5. VERIFY THE FLOOR ─────────────────────────────────────
-- Before any real person uses this app:
-- 1. Create two accounts: one leader, one member.
-- 2. Write a journal entry as the member.
-- 3. Sign in as the leader and run: select * from journal_entries;
-- 4. You should get 0 rows. If you get any rows, STOP.
